...
class RecalculationServiceTest {
    ...
    @Test
    void testCalculatePoints_KnockoutDraw_CorrectAdvancingTeam_10Points() throws Exception {
        match.setPhase(com.worldJackpot.api.model.enums.MatchPhase.ROUND_16);
        match.setHomeScore(1);
        match.setAwayScore(1);

        com.worldJackpot.api.model.Team advancingTeam = new com.worldJackpot.api.model.Team();
        advancingTeam.setId(1L);
        match.setPenaltyWinner(advancingTeam);

        Bet bet = new Bet();
        bet.setHomeScore(0);
        bet.setAwayScore(0);
        
        com.worldJackpot.api.model.Team selectedWinner = new com.worldJackpot.api.model.Team();
        selectedWinner.setId(1L);
        bet.setSelectedWinner(selectedWinner); // Match

        invokeCalculatePointsForBet(match, bet);

        // 5 points for matching draw + 5 points for correctly guessing advancing team = 10 points
        assertEquals(10, bet.getPointsEarned());
    }

    @Test
    void testCalculatePoints_KnockoutDraw_IncorrectAdvancingTeam_5Points() throws Exception {
        match.setPhase(com.worldJackpot.api.model.enums.MatchPhase.ROUND_16);
        match.setHomeScore(1);
        match.setAwayScore(1);
        ...
        com.worldJackpot.api.model.Team selectedWinner = new com.worldJackpot.api.model.Team();
        selectedWinner.setId(2L);
        bet.setSelectedWinner(selectedWinner); // Mismatch

        invokeCalculatePointsForBet(match, bet);

        // 5 points for matching draw + 0 points for incorrectly guessing advancing team = 5 points
        assertEquals(5, bet.getPointsEarned());
    }

    @Test
    void testCalculatePoints_KnockoutDraw_ExactScore_CorrectAdvancingTeam_15Points() throws Exception {
        match.setPhase(com.worldJackpot.api.model.enums.MatchPhase.FINAL);
        match.setHomeScore(2);
        match.setAwayScore(2);

        com.worldJackpot.api.model.Team advancingTeam = new com.worldJackpot.api.model.Team();
        advancingTeam.setId(2L); // Away wins on penalties
        match.setPenaltyWinner(advancingTeam);

        Bet bet = new Bet();
        bet.setHomeScore(2);
        bet.setAwayScore(2); // Exact score
        
        com.worldJackpot.api.model.Team selectedWinner = new com.worldJackpot.api.model.Team();
        selectedWinner.setId(2L); // Correct advancing team
        bet.setSelectedWinner(selectedWinner);

        invokeCalculatePointsForBet(match, bet);

        // 10 points for exact score + 5 points for correctly guessing advancing team = 15 points
        assertEquals(15, bet.getPointsEarned());
    }
}
